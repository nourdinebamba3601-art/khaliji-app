...
import { Save, Image as ImageIcon, Plus, X, Loader2, ArrowRight, Star, GripVertical, Info } from 'lucide-react';
...
export default function EditProductPage(props: { params: Promise<{ id: string }> }) {
    ...
    useEffect(() => {
        ...
        if (product) {
            ...
            setSalesCount(product.salesCount?.toString() || '50');

            // Glasses defaults if available
            if (product.gender) setGender(product.gender);
            if (product.lensType) setLensType(product.lensType);
            if (product.frameMaterial) setFrameMaterial(product.frameMaterial);
            if (product.frameShape) setFrameShape(product.frameShape);
            if (product.hasFullSet !== undefined) setHasFullSet(product.hasFullSet);

            // Watch defaults
            if (product.watchMovement) setWatchMovement(product.watchMovement);
            if (product.strapMaterial) setStrapMaterial(product.strapMaterial);
            if (product.caseMaterial) setCaseMaterial(product.caseMaterial);

            // Perfume defaults
            if (product.perfumeLongevity) setPerfumeLongevity(product.perfumeLongevity);
            if (product.perfumeVolume) setPerfumeVolume(product.perfumeVolume);
            if (product.perfumeScent) setPerfumeScent(product.perfumeScent);
            if (product.videoUrl) setVideoUrl(product.videoUrl);
            ...
            // Products might not be loaded yet from LS in Context, wait a bit
            // In a real app we'd have a loading state from context
    ...
    if (isLoading) {
        return <div className="flex items-center justify-center min-h-[500px] text-white">جاري تحميل بيانات المنتج...</div>;
    }
        ...
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
                    <button onClick={() => router.back()} className="p-2 bg-dark-800 rounded-lg hover:bg-dark-700 text-white transition">
                        <ArrowRight className="w-5 h-5" />
                    </button>
                    <h1 className="text-2xl font-bold text-white">تعديل المنتج: {name}</h1>
                </div>
                    ...
                    <div className="bg-dark-800 p-6 rounded-xl border border-dark-700">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input type="text" value={name} onChange={(e) => setName(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">اسم المنتج (إنجليزي - اختياري)</label>
                                <input type="text" value={nameEn} onChange={(e) => setNameEn(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none text-left" />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">الماركة / البراند</label>
                                <input type="text" value={brand} onChange={(e) => setBrand(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">وصف المنتج</label>
                                <textarea rows={4} value={description} onChange={(e) => setDescription(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none"></textarea>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-dark-800 p-6 rounded-xl border border-dark-700">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="number" value={originalPrice} onChange={(e) => setOriginalPrice(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm text-gold-400 mb-1">سعر البيع (أوقية) <span className="text-red-500">*</span></label>
                                <input type="number" value={price} onChange={(e) => setPrice(e.target.value)} className="w-full bg-dark-900 border border-gold-400/50 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none font-bold" />
                            </div>
                            <div>
                                <label className="block text-sm text-gray-400 mb-1">الكمية المتوفرة</label>
                                <input type="number" value={quantity} onChange={(e) => setQuantity(e.target.value)} className="w-full bg-dark-900 border border-dark-700 rounded-lg px-4 py-2 text-white focus:border-gold-400 outline-none" />
                            </div>
                        </div>
                    </div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                                    <div className="space-y-4">
                                        <p className="text-xs text-gray-500 leading-relaxed">
                                            ارفع فيديو قصير (Reels Style) يوضح فخامة العناية وتغليفها. يشجع هذا الفيديو الزبائن على الشراء بنسبة 80% أكثر.
                                        </p>
                ...
                <div className="space-y-6">
                        ...
                        <div className="space-y-4">
                            <label className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition ${source === 'local' ? 'border-green-500 bg-green-500/10' : 'border-dark-600 bg-dark-900/50 hover:border-green-500/50'}`}>
                                <div className="flex items-center gap-3">
                                    <input type="radio" name="source" value="local" checked={source === 'local'} onChange={() => setSource('local')} className="accent-green-500" />
                                    <span className="text-sm font-bold text-white">متاح في متجرنا</span>
                                </div>
                            </label>
                            <label className={`flex items-center justify-between p-3 rounded-lg border cursor-pointer transition ${source === 'dubai' ? 'border-gold-400 bg-gold-400/10' : 'border-dark-600 bg-dark-900/50 hover:border-gold-400/50'}`}>
                                <div className="flex items-center gap-3">
                                    <input type="radio" name="source" value="dubai" checked={source === 'dubai'} onChange={() => setSource('dubai')} className="accent-gold-400" />
                                    <span className="text-sm font-bold text-white">شحن من دبي</span>
                                </div>
                            </label>
                            {source === 'dubai' && (
                                <div className="pt-2">
                                    <label className="block text-xs text-gold-400 mb-1">مدة الشحن</label>
                                    <input type="text" value={shippingDuration} onChange={(e) => setShippingDuration(e.target.value)} className="w-full bg-dark-900 border border-dark-600 rounded-lg px-3 py-2 text-white text-sm" placeholder="5-7 أيام" />
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-dark-800 p-6 rounded-xl border border-dark-700">
                        ...
                        <div className="space-y-2">
                            {['perfumes', 'watches', 'glasses'].map(cat => (
                                <label key={cat} className="flex items-center gap-2 p-2 rounded hover:bg-dark-700 cursor-pointer">
                                    <input type="radio" value={cat} checked={category === cat} onChange={() => setCategory(cat)} className="accent-gold-400" />
                                    <span>{cat === 'perfumes' ? 'عطور' : cat === 'watches' ? 'ساعات' : 'نظارات'}</span>
                                </label>
                            ))}
                        </div>
                    </div>
